...
int statistique::Stat_partie1()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from EMISSION where temps BETWEEN '0' AND '1'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Stat_partie2()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from EMISSION where temps BETWEEN '2' AND '3'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Stat_partie3()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from EMISSION where temps BETWEEN '4' AND '100'") ;
    while(requete.next())
    {
            count++ ;
    }
...
void statistique::paintEvent(QPaintEvent *)
{
    ...
    ui->label_1->setText("durre  0-1") ;
    painter.setBrush(Qt::green);
    painter.drawPie(size,16*x,16*y);
    ui->label_2->setText("duree 2-3") ;
    painter.setBrush(Qt::blue);
    painter.drawPie(size,16*(y+x),16*z);
    ui->label_3->setText("duree 4-100") ;
